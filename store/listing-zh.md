...
🖱️ 捲動即播放頭、🔴 即時 = DOM 變化（v0.4+）
再多兩種播放方式。捲動模式下，你的捲動位置就是時間軸——往下捲往前播放，往回捲就靜音，再往下捲則重新觸發，像在拖曳影片進度條。即時模式下，網頁本身的即時活動就是演奏：底層持續循環一段安靜的氛圍音床，網頁上元素的新增、移除、屬性變化則疊加成音符——安靜的網頁就保持安靜，活躍的網頁就變成一場真正的演出。

⬇️ 匯出 WAV + MIDI（v0.5）
把目前這首曲子離線算圖——用跟你剛剛聽到的完全相同的合成路徑——匯出成標準 16-bit WAV 檔，全程在本機完成。

MIDI 會匯出同一份確定性樂譜，並保留穩定的 WSE 聲部軌道，方便檢查與後續編輯。WAV 會套用目前的聲部混音；只改混音時，MIDI 與樂譜不會改變。

🎛️ 自訂
五種風格（Ambient、Piano、Electronic、Orchestral、Eastern 東方）、三種模式（Hybrid、Musical、Analytical）、受限的 Mapping Profiles、本機 Presets，以及節奏、密度、亮度、殘響和包含低音的聲部混音控制。23 種合成樂器，包含蕭、笛、clarinet、箏、marimba、subbass 與 choir——不需下載音色庫，全部即時 Web Audio 合成。
